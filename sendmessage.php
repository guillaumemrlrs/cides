...
if ($_POST) {
  ...
  if ($nombreErreur==0) { // S'il n'y a pas d'erreur
    ...
    $destinataire = 'g.morlieras@gmail.com'; // Adresse email du webmaster
    $sujet = 'Titre du message'; // Titre de l'email
    $contenu = '<html><head><title>Titre du message</title></head><body>';
    $contenu .= '<p>Bonjour, vous avez reçu un nouveau message à partir de votre site web !</p>';
    $contenu .= '<p><strong>Nom</strong>: '.$nom.'</p>';
    $contenu .= '<p><strong>Ville</strong>: '.$ville.'</p>';
    $contenu .= '<p><strong>Email</strong>: '.$email.'</p>';
    $contenu .= '<p><strong>Numéro de téléphone</strong>: '.$tel.'</p>';
    ...
    echo "<script>alert(\"Message envoyé !\")</script>"; // Afficher un message pour indiquer que le message a été envoyé
